...
8. Pages 아티팩트 배포 + Vercel 미러 배포(둘 다 같은 `public/`, 항상 최신). 커밋은 `heroes.json`·`maps.json`이 바뀔 때만(신규 영웅/맵) → 평소 일일 커밋 없음
...
- **번역·요약**: Cerebras API (`gpt-oss-120b`)
- **프론트엔드**: Vanilla JS (ES Modules), Chart.js, Tailwind CSS
- **배포**: GitHub Pages(정식) + Vercel 미러 — GitHub Actions가 동일한 `public/`을 양쪽에 배포
